...
import { Get_All_Doc } from '../../../utils/ConstUrls';
...
function Doctorslist() {
  ...
  const getDoctorsReq = async () => {
    try {
      const response = await adminInstance.get(Get_All_Doc, {
  ...
  const viewMore=(id)=>{
    navigate(`/admin/doctorslistdetails/${id}`)
  }
  ...
  return (
    <>
    <h1 className="text-3xl font-semibold text-customGreen ml-1">Doctors List</h1>
        ...
        <label htmlFor="table-search" className="sr-only">
          ...
          {doctorReq&&doctorReq.length>0 ?(
            ...
            <p className="text-gray-500">
              There are no doctor  to display at the moment.
            </p>
          </div>
        )}
