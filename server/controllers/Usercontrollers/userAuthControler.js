...
export const resetPasswordVerification=async(req,res)=>{
  try {
    ...
    if(!user ){
      return res.status(404).json({message:"Acount is not found! Enter a valid emailId"})
    }
...
console.log(`This is the url for verifying user for reseting password ${ResetVerifyUrl}`);
...
const mailOption = {
  ...
  <p style="color: #0070c9; font-weight: bold;">The Docucare Team</p>
       ...
       await transporter.sendMail(mailOption);
       return res.status(201).json({
         message: `Check your email for reseting your password`,MailSend:true
      });
...
export const ResetPasswordAccountVerify=async(req,res)=>{
  ...
  try {
    ...
    return res.status(200).json({
      message: "Account Verified",userId
    });
  } catch (err) {
    // Handle token verification errors
    console.error("Token Verification Error:", err);
